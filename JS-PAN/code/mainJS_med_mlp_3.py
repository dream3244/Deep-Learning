        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            if training_iterator._finished_epoch != epoch:
                ...
                for test_imgs in testiterator:
                    ...
                    label = test_imgs_label.detach().cpu().numpy()

                    # AUC
                    prob = score.detach().cpu().numpy()
                ...
                print("[Epoch %d/%d] [D loss: %f] [R loss: %f] [Accuracy: %f] [precision: %f] [recall: %f] [F1: %f] [AUC: %f]" % (
                    training_iterator._finished_epoch, opt.n_epochs,
                    loss_max_d, loss_r, 1 - acc, precision, recall, f1, AUC))

                f.writelines(f"{loss_max_d},{loss_r},{1 - acc},{f1},{AUC}\n")
